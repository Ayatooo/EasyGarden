...
<form wire:submit="{{ $action }}" class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="space-y-6">
            ...
            <flux:input wire:model="name" label="Nom de la plante" placeholder="Par exemple : Ficus elastica" class="text-sm" />
            <flux:input wire:model="watering_frequency" type="number" label="Fréquence d'arrosage (jours)" placeholder="Par exemple : 2" class="text-sm" />
        </div>
        ...
        <div class="space-y-6">
            ...
            <flux:select wire:model="soil_type" label="Type de sol" placeholder="Sélectionnez un type de sol" class="text-sm">
                @foreach (Plant::SOIL_TYPE_OPTIONS as $type)
                <flux:select.option value="{{ $type }}">{{ $type }}</flux:select.option>
                @endforeach
            </flux:select>

            <flux:select wire:model="sun_exposure" label="Exposition au soleil" placeholder="Sélectionnez une exposition" class="text-sm">
